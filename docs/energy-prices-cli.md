...
| `src/pages/api/admin/diagnostics/[report].ts`         | Admin API route: `GET /api/admin/diagnostics/<report>?...` → `{ ok, data, text }`. Also has a small adapter wrapping the server's in-process GraphQL executor in the same client shape, so it reuses the report modules unchanged |
