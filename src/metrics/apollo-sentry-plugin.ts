...
export const createSentryMetricsPlugin = (
  ...
  resolversEnabled: boolean = false
): ApolloServerPlugin => {
  if (!enabled) {
    return {};
  }
    ...
    async requestDidStart(requestContext) {
      ...
      return {
        ...
        async willSendResponse() {
          ...
          // Track cache metrics
          const cacheHit = requestContext.metrics.responseCacheHit;

          // Record cache metrics as attributes on the operation span
          operationSpan.setAttribute("cache_status", cacheHit ? "HIT" : "MISS");
