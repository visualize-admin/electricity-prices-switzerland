...
import { Maybe, SunshineDataIndicatorRow, OperatorObservation } from "src/graphql/queries";
...
> = {
  networkCosts: (values) => mean(values.filter((v): v is number => v !== null && v !== undefined)) ?? null,
  netTariffs: (values) => mean(values.filter((v): v is number => v !== null && v !== undefined)) ?? null,
  energyTariffs: (values) => mean(values.filter((v): v is number => v !== null && v !== undefined)) ?? null,
  saidi: (values) => mean(values.filter((v): v is number => v !== null && v !== undefined)) ?? null,
  saifi: (values) => mean(values.filter((v): v is number => v !== null && v !== undefined)) ?? null,
...
export const aggregateSunshineObservationsByOperator = (
  observationsByOperatorMap: Map<string, SunshineDataIndicatorRow[]> | undefined,
  ...
  return Object.fromEntries(
    Array.from(observationsByOperatorMap.entries()).map(([operatorId, observations]) => [
      operatorId,
      {
        ...observations[0],
        value: aggregateFn(observations.map((obs) => obs.value).filter((v) => v !== undefined)),
      },
    ]),
  );
};
  ...
  return Object.fromEntries(
    Array.from(observationsByOperatorMap.entries()).map(([operatorId, observations]) => [
      operatorId,
      {
        ...
        value: mean(observations.map((obs) => obs.value).filter((v): v is number => v !== null)) ?? null,
